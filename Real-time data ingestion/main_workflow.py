...
class PatientMonitoringWorkflow:
    def __init__(
        ...
        self.influx = InfluxDBWriter(influx_url, influx_token, influx_org, influx_bucket, simulation_mode=self.simulation_mode)
        
        self.kafka = None
        self.sim_queue = None
        
        if self.simulation_mode:
            logger.info("MAIN WORKFLOW: SIMULATION MODE ACTIVE. Bypassing Kafka.")
            self.sim_queue = queue.Queue(maxsize=100)
        else:
            try:
                self.kafka = KafkaProdCons(bootstrap_servers=kafka_servers, topic=topic)
            except Exception as e:
                logger.error(f"Kafka initialization failed: {e}. Set SIMULATION_MODE=true to bypass.")
                raise
        ...
        try:
            while not self.stop_event.is_set() and (duration is None or duration <= 0 or (time.time() - start) < duration):
                ...
                for patient_id, simulator in self.simulators.items():
                    ...
                    if self.sim_queue:
                        try:
                            self.sim_queue.put_nowait(reading)
                        except queue.Full:
                            pass
                    sent += 1
    ...
    def _process_reading(self, reading: Dict) -> bool:
        try:
            ...
            self.influx.write_metric(
                "model_performance", 
                {"patient_id": reading.get("patient_id"), "model": "PredictorV2"},
                {"latency_ms": latency, "processed": 1}
            )
    ...
    def consumer_loop(self) -> None:
        ...
        while not self.stop_event.is_set():
            if self.kafka:
                # Polling approach to allow checking stop_event
                records = self.kafka.consumer.poll(timeout_ms=500)
                for msg_list in records.values():
                    for message in msg_list:
                        if self._process_reading(message.value):
                            processed += 1
            
            if self.sim_queue:
                try:
                    # Non-blocking get to allow loop to rotate
                    reading = self.sim_queue.get(timeout=0.1)
                    if self._process_reading(reading):
                        processed += 1
    ...
    def run(self, duration: Optional[int] = None, interval: float = 1.0) -> None:
        ...
        mode = "ML Inference" if self.predictor.model else "Rule-based Fallback"
