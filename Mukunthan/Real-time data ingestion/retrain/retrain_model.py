...
def run_retrain() -> None:
    ...
    # 8. Log retraining metrics to InfluxDB
    try:
        influx = InfluxDBWriter(INFLUXDB_URL, INFLUXDB_TOKEN, INFLUXDB_ORG, INFLUXDB_BUCKET)
        influx.write_metric(
            "retraining_metrics",
            {"model": "PredictorV2", "status": "deployed" if should_save else "archived"},
            {
                "accuracy": float(new_metrics["accuracy"]),
                "f1": float(new_metrics["f1"]),
                "precision": float(new_metrics["precision"]),
                "recall": float(new_metrics["recall"]),
                "improvement": 1 if (old_metrics and new_metrics["f1"] > old_metrics["f1"]) else 0
            }
        )
        logger.info("Retraining metrics logged to InfluxDB")
    except Exception as exc:
        logger.error("Failed to log retraining metrics: %s", exc)
